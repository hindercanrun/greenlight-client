	...
	namespace Dumping
	{
		...
		void Cmd_DumpMapEnts_f()
		{
			...
			for (int i = 0; i < count; i++)
			{
				...
				std::string outputPath = "game:\\Alpha\\dump\\" + assetName;
		...
		void Cmd_DumpGfxLight_f()
		{
			...
			for (int i = 0; i < count; i++)
			{
				...
				std::string outputPath = "game:\\Alpha\\dump\\lights\\" + assetName;
		...
		void Cmd_DumpLocalizedStrings_f()
		{
			...
			for (std::map<std::string, std::string>::iterator it = groupedStr.begin(); it != groupedStr.end(); ++it)
			{
				...
				std::string outPath = "game:\\Alpha\\dump\\english\\localizedstrings\\" + it->first + ".str";
		...
		void Cmd_DumpStringTables_f()
		{
			...
			for (int i = 0; i < count; ++i)
			{
				...
				std::string outPath = "game:\\Alpha\\dump\\" + assetName;
